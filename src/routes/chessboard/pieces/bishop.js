...
/**
 * Return a list of all possible squares a bishop at (row, column) can move to
 * @param {*} row 
 * @param {*} column 
 * @returns 
 */
export const bishop_possible_square_list = (/** @type {number} */ row, /** @type {number} */ column) => {
    ...
    for (const turn of [0, 1, 2, 3]) {
        for (const possible_next_move of piece_moveset.slice(7*turn, 7*(turn + 1))) {
            ...
            const [diff_row, diff_column] = possible_next_move;
...
/**
 * Highlight all possible squares a bishop at (row, column) can move to 
 * @param {*} row 
 * @param {*} column 
 */
export const bishop_possible_square_highlighted = (/** @type {number} */ row, /** @type {number} */ column) => {

    const possible_square_list = bishop_possible_square_list(row, column);
...
/**
 * Check if square (row, column) is attacked by a bishop
 * @param {*} row 
 * @param {*} column 
 * @param {*} side 
 * @returns 
 */
export const square_attacked_by_bishop = (/** @type {number} */ row, /** @type {number} */ column, /** @type {String} */ side) => {
    const piece_moveset = get(movesets).bishop;
    ...
    for (const turn of [0, 1, 2, 3]) {
        for (const diff of piece_moveset.slice(7*turn, 7*(turn + 1))) {
            ...
            if (in_boundaries(next_row, next_column)) {
                ...
                if (next_square_info.has_piece && next_square_info.side !== side && next_square_info.name !== 'bishop') break;

                if (next_square_info.has_piece && next_square_info.side !== side && next_square_info.name === 'bishop') is_attacked = true;
